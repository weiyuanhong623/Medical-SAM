...
class Adapter_Layer(nn.Module):
    def __init__(self, embed_dim, mlp_ratio=0.25, norm_layer = nn.LayerNorm, skip_connect=True):
        ...
        self.max_pool = nn.AdaptiveMaxPool2d(1)         #improve
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.channel = nn.Sequential(
                nn.Conv2d(in_channels = embed_dim, out_channels= hidden_dim , kernel_size=1 , bias=False),      #improve
                nn.ReLU(),
                nn.Conv2d(in_channels=hidden_dim, out_channels=embed_dim, kernel_size=1, bias=False),            #improve
                nn.sigmoid()
        ...
        self.depth_conv=nn.Conv2d(in_channels=embed_dim,        #improve
        ...
        self.point_conv = nn.Conv2d(in_channels=embed_dim,      #improve
                                    out_channels=embed_dim,
                                    kernel_size=1,
                                    stride=1,
                                    padding=0,
    ...
    def forward(self, x):
        ...
        x = x.permute(0,3,1,2)
        B, C, _, _ = x.size()
        x_channel = self.channel(self.max_pool(x) * self.avg_pool(x)) * x      #improve
        x_spatial = self.spatial(x_channel)
        
        if self.skip_connect:
            dsc_out = self.depth_conv(x)
            dsc_out = self.point_conv(dsc_out)          #improve
            x = x + x_spatial + dsc_out
        else:
            x = x_spatial
        #（B, C, H, W） -> (B, H, W, C)
        x = x.permute(0,2,3,1)
